...
void ofApp::update() {
	...
	for (int x = -radius * 2.5; x <= radius * 2.5; x += radius * 2.5) {

		for (int y = -radius * 2.5; y <= radius * 2.5; y += radius * 2.5) {
			...
			for (int i = 0; i < this->triangle_list.size(); i++) {
				...
				auto noise_value = ofNoise(x + avg.x * 0.0025 + ofGetFrameNum() * 0.003, (y + avg.y) * 0.0025, avg.z * 0.0025);
				...
				for (auto& vertex : vertices) {
					...
					auto rotation_x = glm::rotate(glm::mat4(), ofMap(ofNoise(glm::vec2(y * 0.007, (x + vertex.x) * 0.0007 + ofGetFrameNum() * 0.0035)), 0, 1, -180, 180) * (float)DEG_TO_RAD, glm::vec3(1, 0, 0));
